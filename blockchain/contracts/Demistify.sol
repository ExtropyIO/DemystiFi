...
function getDetailsForWallet(address _walletAddress) external view returns(uint64 walletScore_, string memory IPFSDetails_){
    return(wallets[_walletAddress].WalletScore,wallets[_walletAddress].IPFSDetails );
}
...
function addWallet(address _walletAddress,  string calldata _IPFSDetails) external onlyOwner walletDoesNotExist(_walletAddress){
    
    walletArray.push(_walletAddress);
    wallets[_walletAddress] = walletDetails({ WalletScore : 50, IPFSDetails : _IPFSDetails, Initialised : true});
    emit WalletAdded(_walletAddress,_IPFSDetails);
    
}

function updateScore(address _walletAddress, uint64 _newScore) external onlyOwner walletExists(_walletAddress) {
...
function deleteWallet(address _walletAddress) external onlyOwner walletExists(_walletAddress) {
